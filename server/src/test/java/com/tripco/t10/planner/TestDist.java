...
public class TestDist {
  ...
  public void testParseLat() {
    assertEquals(88.2 + (20.34 / 60) + (38.1 / 3600.0), distance.parseLatLong("88.2°20.34'38.1\"N", true),.00000001);  //test no spaces
    assertEquals(43.35 + (32.0 / 60) + (42.12 / 3600.0), distance.parseLatLong("43.35° 32' 42.12\" N", true),.00000001);  //test latitude general form
    assertEquals((-43.35 - (32.0 / 60.0) - (42.12 / 3600.0)), distance.parseLatLong("43.35° 32' 42.12\" S", true),.00000001); //test negative sign
    assertEquals((32.0 / 60.0) + (42.12 / 3600.0), distance.parseLatLong("32' 42.12\" N", true),.00000001);  //test exclude degrees from expression
    assertEquals(-32.21, distance.parseLatLong("32.21 S", true),.00000001);  //test decimal degrees plus direction
  }

  @Test
  public void testParseLong() {
    assertEquals(148.2+(20.64/60.0)+(38.1/3600.0), distance.parseLatLong("148.2° 20.64' 38.1\" E",false),.00000001); //test longitude general form
    assertEquals(-31.2-(20.0/60.0)-(100.12/3600.0), distance.parseLatLong("31.2° 20' 100.12\" W",false),.00000001);  //test west and carryover from seconds
    ...
    assertEquals(143.95 + (32.1/60.0), distance.parseLatLong("143.95° 32.1'",false),.00000001);  //test no direction in full format

  }

  @Test
  public void testParseLatLongFail() {
    assertEquals(-1000, distance.parseLatLong("89.95°60' 43\" N",true),.00000001);  //test that latitude cant go over 90
    assertEquals(-1000, distance.parseLatLong("179.95°60' 43\" E",false),.00000001);  //test that longitude cant go over 180;
